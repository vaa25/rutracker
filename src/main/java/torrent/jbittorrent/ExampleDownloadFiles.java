...
class ExampleDownloadFiles {
    ...
    public ExampleDownloadFiles(String[] args) {
        try {
            ...
            TorrentFile torrentFile = tp.getTorrentFile(tp.parseTorrent(args[0]));
            if (args.length > 1) {
                Constants.SAVEPATH = args[1];
            }
            if (torrentFile != null) {
                System.out.println("TorrentFile parsed");
                DownloadManager downloadManager = new DownloadManager(torrentFile, Utils.generateID());
                System.out.println("DownloadManager created");
    ...
    public static void main(String[] args) {
        new ExampleDownloadFiles(new String[]{"[rutracker.org].t5339459.torrent"});
    }
}
