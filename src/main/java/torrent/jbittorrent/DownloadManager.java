...
public class DownloadManager implements DTListener, PeerUpdateListener,
    ...
    private PeerUpdater peerUpdater = null;
...
//                System.out.println("\r\nSharing... Press Ctrl+C to stop client");
    ...
    public void startTrackerUpdate() {
        this.peerUpdater = new PeerUpdater(this.clientID, this.torrent);
    ...
    public synchronized int checkTempFiles() {
        ...
        for (int i = 0; i < this.nbOfFiles; i++) {
            File temp = new File(saveas + this.torrent.name.get(i));
            try {
    ...
    public synchronized void savePiece(int piece) {
        ...
        for (
                Iterator it = this.pieceList[piece].getFileAndOffset().keySet().
                        iterator(); it.hasNext(); ) {
            try {
                ...
                this.output_files[file.intValue()].seek(((Integer)
                        ...
                        intValue());
                this.output_files[file.intValue()].write(
                        data,
     ...
     */
    public synchronized void pieceCompleted(
            String peerID, int i,
            boolean complete
    ) {
        synchronized (this.isRequested) {
        ...
        synchronized (this.isComplete) {
            if (complete && !this.isPieceComplete(i)) {
                peerUpdater.updateParameters(this.torrent.pieceLength, 0, "");
                this.isComplete.set(i, complete);
                float totaldl = ((float) (100.0)) *
                        ((float) (this.isComplete.cardinality())) /
                        ((float) (this.nbPieces));

                for (
                        Iterator it = this.task.keySet().iterator();
                        it.hasNext(); ) {
                    try {
...
//                        System.out.println(p + " optimistically unchoken...");
     ...
     * @param length int
     */
    public synchronized void peerRequest(
            String peerID, int piece, int begin,
            int length
    ) {
        if (this.isPieceComplete(piece)) {
            ...
            dt = null;
            this.peerUpdater.updateParameters(0, length, "");
        } else {
            try {
        ...
        for (
            ...
            try {
                ...
                this.output_files[file.intValue()].seek(((Integer)
                        (this.pieceList[piece].getFileAndOffset().get(file))).
                        intValue());
                this.output_files[file.intValue()].read(
                        data,
                        data.length - remainingData,
                        (remaining < remainingData) ? remaining : remainingData
                );
