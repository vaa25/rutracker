...
public class DownloadTask extends Thread implements IncomingListener,
     ...
     */
    public DownloadTask(
            Peer peer, byte[] fileID, byte[] myID, boolean init, byte[] bitfield,
            Socket s
    ) {
    ...
    public synchronized void messageReceived(Message m) {
        ...
        if (m.getType() == PeerProtocol.HANDSHAKE) {
            ...
            if (Utils.bytesCompare(
                ...
                this.ms.addMessageToQueue(new Message_PP(
                        PeerProtocol.BITFIELD,
                        this.bitfield
                ));
                        ...
                        this.firePeerRequest(this.peer.toString(),
                                Utils.byteArrayToInt(
                                        Utils.subArray(message.
                                                        getPayload(),
                                                0, 4
                                        )), Utils.byteArrayToInt(Utils.subArray(
     ...
     */
    private synchronized void firePieceRequested(
            int piece,
            boolean requested
    ) {
     ...
     */
    private synchronized void firePieceCompleted(
            int piece,
            boolean complete
    ) {
        for (DTListener listener : getDTListeners()) {
     ...
     * @param length int
     */
    private synchronized void firePeerRequest(
            String peerID, int piece,
     ...
     */
    private synchronized void firePeerAvailability(
            String id,
            BitSet hasPiece
    ) {
        for (DTListener listener : getDTListeners()) {
